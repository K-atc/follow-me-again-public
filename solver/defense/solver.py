    ...
    while pc:
        ...
        if instruction.isBranch() and not instruction.getType() == OPCODE.X86.JMP:
            ...
            if False:
                ...
                conditional_branch_constraints.append(selecter(instruction, event, OPCODE.X86.JS, js_succ, OPCODE.X86.JNS, jns_succ)(sf=reg_sf))
            else:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        debug('Syntax: %s <tracer log> <elf binary> <answer file> [arg1, arg2, ...]' %(sys.argv[0]))
        sys.exit(1)
